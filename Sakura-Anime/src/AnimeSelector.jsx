...
function AnimeSelector({ searchTerm, setSearchTerm, onSelectAnime }) {
  ...
  const skipNextFetch = useRef(false) //skip the next fetch if a user selects an anime (no unneccisary api calls :P)
  const wrapperRef = useRef(null); //fo the wrapper div to detcts click outside bc it's annoying to have the make a selcetction or the dropdown WONT GO AWAY AND LEAVE YOU ALONE
  ...
  useEffect(() => {
    ...
    if (!searchTerm || searchTerm.trim().length < 2) { // check if searchTerm exists BEFORE trying to .trim()
      setResults([]);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5050/search?q=${encodeURIComponent(searchTerm)}&fields=${fields}`);
  ...
  // animes is an array of all the anime objects fetched from API.
  ...
  // e.target.value is the value of the selected <option>, which is set as the anime’s id in dropdown.
  // const handleChange = (e) => { //function to handle when the user selescts an anime

  // //     animes is the array of all anime objects fetched from API.
  ...
  // // This updates component’s state to the selected anime object.
  // // When state updates, React re-renders the component so it can display the selected anime info elsewhere.
  ...
  // close dropwpown when clicking outside o7
  useEffect(() => {
    const handler = (e) => {
